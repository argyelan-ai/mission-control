...
class JarvisVoiceAdapter:
    """Jarvis' voice channel as a host harness (ADR-082).
    ...
    * ``build_agent_env`` returns {} — nothing renders an agent.env for
      Jarvis, and ``sync_host_agent_model`` never gets far enough to try:
      it returns early for any protocol outside openai/anthropic, and ours
      is "voice". So no file on disk is touched by a switch, and the API
      keys stay where they belong: in the voice-worker container's own env.
    * ``reload`` is a no-op ON PURPOSE. Restarting the container would cut off
      ...
      from the backend at the start of every call (GET
      /api/v1/agent/voice/config), so a switch takes effect on the next one.
    * ``bootstrap`` refuses. MC does not provision this agent; docker compose
    ...
    There is no launchd job for Jarvis (it's a compose service, not a host
    process) — the generic host-agent process-restart/stop buttons in
    ``cli_terminal.py`` will not find a matching launchd label for it. That is
    a pre-existing UI affordance outside this adapter's scope; the runtime
    switch itself (the thing this adapter exists for) does not depend on it.
