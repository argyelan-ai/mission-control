...
"""
    ...
    if _has(Scope.CHAT_WRITE):
        flow_blocks.append(
            ...
            "\n"
            "# Ad-hoc file WITHOUT a task/deliverable — --vault-path takes a\n"
            "# vault wrapper path (find it via `mc vault-search`)\n"
            "mc report \"Datei anbei\" --vault-path wrappers/files/example.md\n"
            "\n"
    ...
    if _has(Scope.TASKS_CREATE):
        flow_blocks.append(
            ...
            "# Order came from a chat conversation? Add --origin-thread <id> (id from the\n"
            "# message footer) — the server then mirrors the final report into that thread.\n"
            "# Subtasks inherit the origin from their parent automatically.\n"
            "\n"
            "# No active task right now (e.g. handling a chat order cold)? Without --parent\n"
            "# this creates a ROOT card — no parent, no callback, the response says so\n"
            ...
            "# (register final.pdf as a deliverable first — --file takes the UUID, not a path)\n"
            "mc report \"Multi-phase weather report complete. See deliverables.\" --file <deliverable-uuid>\n"
            "mc done\n"
            "```"
        )
