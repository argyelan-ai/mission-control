...
# The only placeholders a template may use.
KNOWN_PLACEHOLDERS = ("port", "model", "slug", "container_name", "image")
...
def build_launch_command(
    *,
    ...
    used. The rendered command always carries ``--label mc.runtime.slug=<slug>``
    (via both the templates and the check below) — without it the lifecycle
    ops can't find the container again.
    """
    ...
    if f"mc.runtime.slug={slug}" not in command:
        raise ValueError(
