...
async def sync_pending_agents(session: AsyncSession, *, force: bool = False) -> None:
    """Sync every flagged agent that is idle (or all flagged when ``force``)."""
    result = await session.exec(
        select(Agent).where(Agent.pending_runtime_sync.is_(True))
    )
    for agent in result.all():
...
async def _sync_one(session: AsyncSession, agent: Agent) -> None:
    ...
    await emit_event(
        session,
        "agent.model_synced",
        f"{agent.name}: now running {runtime.model_identifier} ({runtime.slug})",
        severity="info",
        agent_id=agent.id,
