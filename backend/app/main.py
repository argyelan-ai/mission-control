...
# Architektur E Teil 2: Boot-/Dienst-Pfad extracted to app.background —
# this module no longer defines prepare_process/start_/stop_background_services
# (see app/background.py). background imports app.main only lazily inside
# prepare_process (the seed helpers live here), so the graph stays acyclic.
...
async def lifespan(app: FastAPI):
    ...
    # ── Vault Memory (M.1 Read Foundation + M.2 Write Path) ─────────────
    # Architektur E Teil 2: das komplette Vault-Wiring (Index/Activity/Git/
    # Embeddings/Watcher/Compactor/Lint-Cron) ist nach app.background
    # (start_vault_services/stop_vault_services) extrahiert — dieselbe
    # Funktion bedient jetzt API UND Worker. Die API haengt die Rueckgabe an
    # app.state (Vault-Read-Routen lesen daraus). Failure semantics
    # unveraendert: non-fatal, Backend bootet weiter, Routen 500en.
    from app.background import start_vault_services as _start_vault_services
    _vault_runtime = await _start_vault_services(app)
    for _key, _val in _vault_runtime.items():
        setattr(app.state, _key, _val)
    ...
    if _jarvis_briefing_task is not None:
        ...
        try:
            ...
            pass
    # Vault Memory shutdown — Reihenfolge (lint cron → compactor → watcher →
    # index close) lebt jetzt in app.background.stop_vault_services().
    from app.background import stop_vault_services as _stop_vault_services
    await _stop_vault_services(_vault_runtime)
