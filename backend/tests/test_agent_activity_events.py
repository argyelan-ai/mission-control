    ...
    for offset_days, count in [(1, 3), (3, 2), (6, 1)]:
        for i in range(count):
            await _add_event(
                ...
                created_at=now - dt.timedelta(days=offset_days, hours=i),
            )

    # Noise: different event_type inside the window, and a blocked event outside it
    await _add_event(async_session, board_id=board.id, event_type="task.stuck", created_at=now - dt.timedelta(days=2))
    await _add_event(
        async_session, board_id=board.id, event_type="task.blocked",
        created_at=now - dt.timedelta(days=9),
    )

    resp = await _summary(client, token, event_type="task.blocked")
    assert resp.status_code == 200
    body = resp.json()
...
async def test_summary_bucket_count_is_capped(client: AsyncClient, async_session, monkeypatch):
    """Sabotage probe: remove the `.limit(ACTIVITY_EVENTS_SUMMARY_CAP + 1)` /
    truncation slicing in agent_activity_events_summary -> this test goes
    red (all 5 buckets come back instead of the lowered cap of 3, and
