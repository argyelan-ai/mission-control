"""Tests for scripts/grok-bridge.py MSG_DELIVERY_MODE=nudge (W2.1 nudge+pull).

Port of poll.sh's deliver_messages_nudge contract to the grok bridge:
...
- verify via the unique `(bis seq N, EPOCH)` token OUTSIDE the composer box;
  fail → state unchanged (retry next poll)
...
def _load_bridge():
    spec = importlib.util.spec_from_file_location("grok_bridge_nudge", BRIDGE_PATH)
    mod = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    sys.modules["grok_bridge_nudge"] = mod
...
def _wire_paste(monkeypatch, bridge, *, echo_token=True):
    """Fake paste_and_submit + capture_pane: after a paste, the pane shows the
    nudge line as a submitted transcript line (echo_token=True) or shows
    nothing (echo_token=False → verify must fail). Returns the pasted texts."""
    pastes: list[str] = []
...
def test_token_in_composer_box_is_not_delivery(bridge):
    token = "(bis seq 2, 1234)"
    pane_composer = f"transcript\n│ ❯ 📬 Neue Nachrichten {token} — … │\n"
    pane_submitted = f"📬 Neue Nachrichten {token} — lies sie jetzt mit: mc inbox\n❯ "
    assert bridge._nudge_token_visible(pane_composer, token) is False
...
def test_paste_mode_default_never_calls_nudge(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mod, "msg_gate_open", lambda **kw: False)
...
# ── launch shell exports MC_API_URL for agent-side mc inbox ────────────────────


def test_launch_shell_exports_mc_api_url_default(bridge):
    cmd = bridge._grok_launch_shell_cmd()
    assert 'MC_API_URL:=http://localhost:8000' in cmd
    assert "export MC_API_URL" in cmd
    # agent.env still sourced first — its value wins over the default.
    assert cmd.index("set -a") < cmd.index("MC_API_URL")
