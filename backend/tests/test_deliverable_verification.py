"""Tests: deliverable verification endpoints (Bug A + E fix).

Incident context 2026-04-23 (root task e00d2932 "Kleine Rescherche"):
Agents couldn't verify that `content` was saved after POST /deliverables —
the LIST endpoint hid `content`, and there was no single GET. Result: the
researcher re-registered the same deliverable 4x, and Boss triggered the
done→inbox crash during phase_rewrite.

These tests cover:
  - Bug A: `?include_content=true` on LIST, new single-GET endpoint
  - Bug E: dedup on POST (same agent+task+path returns the existing one)
"""
...
async def test_list_deliverables_omits_content_by_default(client, fake_redis):
    """LIST response has no content field, but content_length shows the size."""
...
async def test_list_deliverables_with_include_content_returns_full_body(client, fake_redis):
    """LIST with ?include_content=true returns the full content body."""
    from app.models.deliverable import TaskDeliverable
    board_id, agent_id, task_id, token = await _setup_agent_and_task()
...
async def test_single_get_deliverable_returns_content(client, fake_redis):
    """New single-GET endpoint returns content + meta fields."""
    from app.models.deliverable import TaskDeliverable
    board_id, agent_id, task_id, token = await _setup_agent_and_task()
...
async def test_single_get_deliverable_404_for_wrong_task(client, fake_redis):
    """Deliverable belongs to another task → 404."""
    from app.models.deliverable import TaskDeliverable
    from app.models.task import Task
    board_id, agent_id, task_id, token = await _setup_agent_and_task()
    ...
    # Access via task_id (not other_task) must return 404
    resp = await client.get(
        f"/api/v1/agent/boards/{board_id}/tasks/{task_id}/deliverables/{deliv_id}",
        headers={"Authorization": f"Bearer {token}"},
    )
...
async def test_create_deliverable_dedup_returns_existing(client, fake_redis, tmp_path):
    """Second POST with the same path+title+agent returns the existing deliverable."""
    from app.models.deliverable import TaskDeliverable
    board_id, agent_id, task_id, token = await _setup_agent_and_task()
    ...
    # Second call with the same path → dedup
...
async def test_create_deliverable_dedup_per_agent(client, fake_redis):
    """A different agent on the same task+path is NOT a duplicate (legitimate cross-agent contributions)."""
    from app.models.deliverable import TaskDeliverable
    from app.models.board import Board
    from app.models.agent import Agent
    from app.models.task import Task
    ...
    # Second agent on the same board + task
    ...
    # Agent B posts the same path → should create NEW (no dedup)
