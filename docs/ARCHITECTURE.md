...
| `heads/night_shift.py` (ROADMAP E2) | Nachtschicht: startet als „run tonight“ markierte Aufgaben im Zeitfenster (`night_shift_*` in app_settings, Standard 22:00–06:00, aus) nacheinander über den Head-Starter (`heads/start.py`), ein Head pro GPU-Box, Cloud-Anteil nur bei Cloud-Paaren; meldet blockierte Heads (Frage oder 15 min ohne Lebenszeichen) einmal und schickt einen Morgenbericht über `operator_reports.send_report`. Marken als Dateien unter `heads_root/night/` (keine Migration, kein FK) | 60s |
...
**Telemetrie-Verlauf 1h (Bühne v2 §6/§7 PR 3, `services/host_metrics_history.py`):** kein zweiter Sammler — jeder erfolgreiche `GET /hosts/{id}/metrics`-Aufruf mit `kind` in (`ssh`, `agent`) und `reachable=true` schreibt einen Punkt in den Redis-Ring `mc:host:{host_id}:metrics:history` (dedupliziert auf max. 1 Punkt/4s, `LTRIM` auf 720 Punkte = 1h @ 5s-Poll). Gelesen über `GET /hosts/{id}/metrics/history?window=` → `{points, window, sample_seconds}`, leerer Ring → `points: []`, nie 5xx. Der optionale Hintergrund-Sampler aus der Spec (`HOST_METRICS_HISTORY_SAMPLER`) wurde bewusst nicht gebaut: für `kind=ssh` wäre er eine SSH-Verbindung pro Host alle 5s unabhängig von offenen Browsern — teurer als der bestehende Frontend-Poll rechtfertigt (siehe PR-Beschreibung).
