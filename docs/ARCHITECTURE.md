...
**Runtime-Adapter-Kontrakt (W2.1, ADR-071):** poll.sh spricht jedes CLI (claude/openclaude/…) über vier Funktionen an — `detect_turn_state` / `paste_and_submit` / `verify_paste` / `detect_pane_ui`, byte-identisch handgepflegt in `docker/mc-agent-base/lib/` + `docker/mc-claude-agent/lib/`. Turn-Erkennung folgt einer Signal-Hierarchie: 1. natives Signal/Hook (claude-code `UserPromptSubmit`/`Stop` → `~/.turn-signal`, `TURN_SIGNAL_MODE=auto`) → 2. strukturierter Output → 3. Pane-Scraping (fragil, PFLICHT-Fallback). Golden-Fixture-TCK `backend/tests/test_adapter_tck.py` (Fixtures unter `backend/tests/fixtures/panes/<cli>/`, Recorder `tools/record-pane-fixtures.sh`) pinnt die Scraping-Schicht pro CLI, damit ein CLI-Update ein roter Test statt eine kaputte Flotte wird. Vollständiger Kontrakt + Onboarding-Checkliste: **[`docs/adapters.md`](adapters.md)**.
...
- **Lesen:** Backend, Frontend `/memory` (Liste + Graph Tabs), Jarvis (OpenAI Realtime via voice-worker, xAI-Fallback siehe ADR-060), Obsidian-App
...
- Jarvis (host-runtime, OpenAI/xAI Realtime worker im voice-worker Container; früher "Voice", siehe ADR-038; Provider-Switch siehe ADR-060; kanal-agnostisch + Telegram-Inbound siehe ADR-061) — `jarvis_core/mc_client.py:vault_*` Function-Tools (per `git mv` aus `voice_worker/`), kein TOOLS.md nötig
...
| Host-Agent-Staging (`.plist`/`run.sh`/`agent.env`) | `backend/app/services/host_provisioning.py` | ADR-063 — nur für generische Host-Agenten (nicht Hermes, das bleibt `bootstrap_hermes_agent`); `launchctl`-Load hinter `settings.host_agent_autoload_enabled` |
...
- **2026-07-22** — **W2.1 Delivery Foundation — native Turn-Signale, pull-basierte Zustellung, Adapter-TCK (ADR-071):** Dreistufiges Fundament für sichere Zustellung, byte-identische Flotte. (A) claude-code-Hooks (`UserPromptSubmit`/`Stop`) schreiben `<epoch> submit|stop` in `~/.turn-signal`; `detect_turn_state` liest sie zuerst (`TURN_SIGNAL_MODE=auto`), Pane-Scraping bleibt PFLICHT-Fallback (`Stop` feuert nicht bei Interrupt/Crash → `crashed` scrape-autoritativ). (B) `mc inbox`-Nudge + Pull-Delivery. (C) 4-Funktionen-Adapter-Kontrakt (`detect_turn_state`/`paste_and_submit`/`verify_paste`/`detect_pane_ui`) mit Signal-Hierarchie (natives Signal → strukturierter Output → Pane-Scraping+Fixtures), neuer Golden-Fixture-TCK `backend/tests/test_adapter_tck.py` (parametrisiert über `backend/tests/fixtures/panes/<cli>/` — neues CLI anbinden = Fixtures aufnehmen, Suite läuft automatisch mit), Fixture-Recorder `tools/record-pane-fixtures.sh` (nimmt Panes live aus Containern auf), Byte-Identitäts-Guard der zwei handgepflegten Lib-Kopien (`docker/mc-agent-base/lib/` + `docker/mc-claude-agent/lib/`). Die TCK deckte sofort eine reale Scraping-Lücke auf: claude-cli 2.1.x rendert die Eingabebox mit bare `❯` auch mitten im Turn, sodass der Idle-Check im Scrape-Modus einen aktiven Turn als `idle` klassifiziert (in Prod vom Phase-A-Signal maskiert) — als `xfail(strict)` kodiert, Fix gehört in `turn-state.sh`. Kontrakt + Onboarding-Checkliste: `docs/adapters.md`. ADR-071.
...
- **2026-08-21** — **Jarvis' Sprach-Anbieter ist eine Runtime-Bindung (ADR-074, ersetzt ADR-060):** Jarvis war der einzige Agent mit `runtime_id=NULL` — welcher Anbieter sprach, stand nur in `VOICE_PROVIDER` in der Container-Env, unsichtbar in MC und nur per compose-Edit + Rebuild änderbar. Jetzt: zwei Seed-Anker-Zeilen (`voice-openai`, `voice-xai`, Muster `grok-cloud`), neues Protokoll `"voice"` in `harness_compat` (verhindert, dass ein CLI-Harness sie bindet oder Jarvis eine Chat-Runtime), `JarvisVoiceAdapter` in `HOST_ADAPTERS` (macht den Picker auf; `reload()` startet bewusst NICHTS neu). Wirkmechanismus ist ein Pull pro Anruf: `voice_worker/main.py::entrypoint()` holt `GET /api/v1/agent/voice/config` beim Sitzungsstart — LiveKit gibt pro Anruf einen frischen Raum, also wirkt ein Wechsel ab dem nächsten Anruf ohne Neustart. Schlüssel bleiben ausschliesslich in der Container-Env (die Route liefert nie Schlüsselmaterial). Env = Notfall-Default; Stimme jetzt pro Arm (`VOICE_OPENAI_VOICE_ID`/`VOICE_XAI_VOICE_ID`, die Stimmnamen der Anbieter sind disjunkt). Leitregel im Worker: Jarvis wird nicht stumm — unbekannter Anbieter → env-Default, fehlender Schlüssel → der Arm mit Schlüssel, nur ohne jeden Schlüssel wird geworfen. **Grenze:** der Schalter stellt NUR den Sprach-Kanal um; Text-Kanal (`brain.py`) und `ask_frontier` sprechen weiterhin fest OpenAI. ADR-074.
