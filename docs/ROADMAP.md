...
| **E1b** — head launcher (harness × runtime, local first) | ADR-086 · "Run as head" on a new task: pick harness × runtime from the compatibility matrix (local runtime pre-selected), MC starts one short-lived head in its own worktree from `origin/main` with the harness-neutral procedure (`AGENTS.md` + prompt) · status (running / needs you / passed / failed), heartbeat, stop, "restart with …" another pair · result (PR + run record) on the task detail · runtimes page shows which runtime is busy · forbidden list and the 30 % usage limit for Claude heads apply · reuses matrix, runtimes table, worktree code and task detail; no persistent agent, no change to frozen dispatch or healers | in progress | One job each on a local pair and a Claude pair ends as PR + run record, started and watched from MC only · an incompatible pair is refused with its reason from the matrix · stop ends the process and the run record says "stopped" · "restart with …" another pair continues the same job · a local engine outage ends as "failed — runtime unreachable", never as done (sabotage: engine stopped mid-run) · no forbidden action reaches the host |
| **Stop 1** (~2 weeks after E0) | Is the new meaning of the north star ("team" = procedure plus short-lived heads) acceptable? Does the path work without the operator at the desk, outside this repo, with a local helper? Does local first hold — what share of jobs finish on a local runtime, and at what operator cost (ADR-086)? Is output-only on chat channels acceptable? | next | Operator decision recorded. **No → exit "harden the fleet":** fleet leaves quiet mode, hardening resumes, new ADR |
| **E2** — unattended runs | Scheduled background sessions without remote control · forbidden list enforced (ADR-085 decision 2) · usage switch (no new heads above the limit share) · stall notice job (reads run-record heartbeats, reports "blocked / stalled" to chat) · automatic fallback local → cloud helper · **started:** night shift on the head launcher — "run tonight" marks, time window in Settings, one head per GPU box, cloud share only for cloud pairs, blocked notice and morning report in MC (Home card; Slack / Telegram copy optional, off by default) (off by default, not merged yet) | in progress (night shift built, not switched on; fallback local → cloud and usage switch not started) | Evening job → PR and run record next morning, also with a simulated GPU-box outage · a job with a deliberately forbidden action produces a visible "blocked" notice by morning · interactive sessions stay logged in |
