...
| [060](060-host-harness-adapter.md) | HostHarnessAdapter — generischer Adapter-Layer für Host-Agent-Runtime-Bindung (`HOST_ADAPTERS`-Registry, voll implementierter `HermesAdapter`, Hermes `custom`-Provider-Render in `~/.hermes/config.yaml`); Auto-Forward (ADR-054) erreicht jetzt auch Host-Agents mit Adapter; host-sicherer In-Place-Runtime-Switch + `single_instance`-Präzisierung als Amendment zu ADR-029 (keine parallele Instanz ≠ kein Umbinden); kein LiteLLM-Shim, `ClaudeCodeHostAdapter` nur designed, Boss unangetastet | Accepted | 2026-07-07 | Backend/Provisioning · Backend/Runtime · Infra/Host-Bridge · Frontend/Runtimes |
