...
1. Message typed to the ACP omp agent appears as user line + streamed preview + final assistant line in the Sessions chat; `docker exec … acp_chat_ctl.py state` shows `turn` incremented.
...
6. The ACP omp agent's Sessions page: no Chat/Terminal toggle; `?view=terminal` still opens the pane.
7. Hermes: chat message answered over `hermes acp`; history survives a bridge restart.

## Rollout

PR A (bridge daemon + ctl + entrypoint + tests) → PR B (backend) → PR C (frontend) → PR D (Hermes). Container recreate of the ACP omp agent only after Boss reports the #148 follow-up task finished; hermes-bridge reload only after Hermes' task G5 — both announced to Boss first.
