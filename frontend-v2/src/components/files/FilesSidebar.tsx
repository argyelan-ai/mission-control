...
function SidebarEntry({
  ...
  return (
    ...
    >
      ...
      {count !== undefined && (
        <span
          className="text-[10px] font-mono tabular-nums px-1.5 py-0.5 rounded-sm shrink-0"
          style={{ background: active ? C.accentSubtle : C.bgElevated, color: active ? C.accent : C.textMuted }}
        >
          {count}
        </span>
      )}
