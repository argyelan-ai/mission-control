...
| File | What it is |
|---|---|
| `entrypoint.sh` | Container PID 1. Bootstraps tokens, renders `models.yml`, skips the omp setup wizard (`omp config set`), then boots the tmux layout: **Win0 = native TUI**, **Win1 = `bridge.py --serve`**, **Win2 = recycler** — plus **Win3 = `acp_chat.py --serve`** when `OMP_DRIVER=acp`. |
